...
# set config_yaml
config_file_path = root_dir_path / 'misc' / 'analysis' / 'performance_metric_time_series_plots' / 'single_intersection' / 'lane_level' / 'config.yaml'
...
save_base_dir_path = data_dir_path / 'analysis' / 'performance_metric_time_series_plots' / 'single_intersection' / 'lane_level'
...
for simulator_dir_path in inflow_dir_path.rglob('simulator_*'):
    ...
    for method_dir_path in mpc_dir_path.glob('config_*'):
        ...
        for road_dir_path in intersection_dir_path.glob('road_*'):
            ...
            rename_column_map = {column: f"road_{road_id}_{re.match(r'queue_([a-z]+)', column).group(1)}" for column in exist_queue_columns}
            tmp_time_series_df = tmp_time_series_df[['time'] + exist_queue_columns].copy()
            tmp_time_series_df = tmp_time_series_df.rename(columns=rename_column_map)

            tmp_time_series_df = tmp_time_series_df.melt(
                id_vars=['time'],
                value_vars=rename_column_map.values(),
                var_name='type',
                value_name='queue',
            )
...
for method, tmp_time_series_df in time_series_df_map.items():
    max_queue_length = max(max_queue_length, tmp_time_series_df['queue'].max())

for method, tmp_time_series_df in time_series_df_map.items():
    # add line_style column
    tmp_time_series_df['line_style'] = 'solid'
    ...
    #     match_obj = re.match(r'road_(\d+)_(main|right|left)', label)
    #     if match_obj:
    #         new_handles.append(handle)
    #         new_labels.append(config_yaml['figure']['legend'][match_obj.group(0)])
