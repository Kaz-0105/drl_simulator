...
class ReplayBuffer (Common):
    ...
    def load(self):
        ...
        self.sum_tree.set('tree', tree_data['tree'])
        self.sum_tree.set('next_data_idx', tree_data['next_data_idx'])
        self.sum_tree.set('current_size', tree_data['current_size'])
        self.new_data_count = tree_data['new_data_count']
...
class SumTree (Common):
    def __init__(self, buffer):
        ...
        self.buffer = buffer  
        self.initial_priority = self.buffer.get('initial_priority')
        
        # ツリーに格納するデータ数の最大値を設定
        self.capacity = self.buffer.get('max_size')
